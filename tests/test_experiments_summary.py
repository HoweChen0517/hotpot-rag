...
def test_summary_only_exposes_title_level_retrieval_metrics(monkeypatch, tmp_path: Path):
    class DummyLLM:
    ...
    assert "retrieval_mrr" in result.summary
    assert "supporting_title_f1" in result.summary
    assert "supporting_sentence_f1" not in result.summary
